...
Page({
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({
          ...
          normalPageX: res.windowWidth * 0.1013,
          ...
          imgWidth: res.windowWidth * 0.57, //user图片宽度
          imgHeight: res.windowWidth * 0.57, //user图片高度
          imgPageY: res.windowWidth * 0.1013, //user图片Y轴位置
          codeWidth: res.windowWidth * 0.2974, //小程序码图片宽度
          codeHeight: res.windowWidth * 0.2974, //小程序码图片高度
          codePageY: res.windowWidth * 1.08, //小程序码Y轴位置
          avatarPageY: res.windowWidth * 0.1013 + res.windowWidth * 0.57 + 108, //地址IconY轴位置
          titlePageY: res.windowWidth * 0.1013 + res.windowWidth * 0.57 + 120, //地址Y轴位置
          specPageY: res.windowWidth * 0.1013 + res.windowWidth * 0.57 + 70, //手机Y轴位置
          pricePageY: res.windowWidth * 0.1013 + res.windowWidth * 0.57 + 35, //姓名职位Y轴位置
          timePageY: res.windowWidth * 0.1013 + res.windowWidth * 0.57 + 95 //门店地址Y轴位置
        });
      }
    });
    //网络图片转为本地图片，直接显示网络图片的话真机不显示
    that.getTempFile("https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602660085408redt_e300.png",1);
    that.getTempFile("https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/160125547050801.png",2);
    that.getTempFile("https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602742769670redt_e300.png",3);
    that.getTempFile("https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602745054504redt_e300.png",4);
    that.getTempFile("https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602742744010redt_e300.png",5);
    // }
  },
  //临时图片路径
  ...
  createNewImg: function (lineNum) {
    ...
    ctx.drawImage(
      that.data.qrCode,
      that.data.normalPageX + that.data.windowWidth * 0.56,
      that.data.codePageY,
      that.data.codeWidth,
    ...
    ctx.fillText(
      // "长按分享给好友",
      that.data.normalPageX +
      that.data.windowWidth * 0.5 +
      (that.data.codeWidth - 58) / 2,
