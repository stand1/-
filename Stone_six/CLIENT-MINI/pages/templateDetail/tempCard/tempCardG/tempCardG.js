// pages/templateDetail/tempCard/tempCardG/tempCardG.js
...
Page({
  data: {
    ...
    imgs: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602840140638redt_e300.png", // 背景图
    name: "段杨杨",
    position: "销售顾问",
    phone: "136 9482 4093",
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({
          ...
          boxWidth: res.windowWidth * 1, //分享图片box宽度
          boxheight: res.windowWidth * (0.222 + 0.72 + 0.192) + 110, //分享图片box高度
          boxPageY: res.windowWidth * 0, //boxY轴位置
          imgWidth: res.windowWidth * 0.96, //user图片宽度
          imgHeight: res.windowWidth * 0.96, //user图片高度
          imgPageY: res.windowWidth * (-0.1313), //user图片Y轴位置
          codeWidth: res.windowWidth * 0.192, //小程序码图片宽度
          codeHeight: res.windowWidth * 0.192, //小程序码图片高度
          codePageY: res.windowWidth * 1.12, //小程序码Y轴位置
          avatarPageY: res.windowWidth * 0.896 + 128, //地址IconY轴位置
          titlePageY: res.windowWidth * 0.896 + 140, //地址Y轴位置
          specPageY: res.windowWidth * 0.896 + 90, //手机Y轴位置
          pricePageY: res.windowWidth * 0.896 + 35, //姓名职位Y轴位置
          timePageY: res.windowWidth * 0.896 + 115 //门店地址Y轴位置
        });
      }
    });
  ...
  createNewImg: function (lineNum) {
    ...
    //user图片
    ctx.arc(that.data.imgWidth / 2 + that.data.windowWidth * 0.1467, that.data.imgHeight / 2 + that.data.imgPageY, that.data.imgWidth / 2, 0, Math.PI * 2, false);
    ctx.clip();
    ctx.drawImage(
      that.data.userImg,
      that.data.windowWidth * 0.1467,
      that.data.imgPageY,
      that.data.imgWidth,
      that.data.imgHeight
    );
