// pages/templateDetail/tempCard/tempCardE/tempCardE.js
...
Page({
  data: {
    ...
    imgs: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602829281371redt_e300.png", // 背景图
    name: "段杨杨",
    position: "销售顾问",
    phone: "136 9482 4093",
    ...
    addressIcon: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602829396180redt_e300.png",
    iconWidth: 14,
    iconHeight: 14
  },
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({
          ...
          normalPageX: res.windowWidth * 0.088,
          ...
          imgWidth: res.windowWidth * 0.4187, //user图片宽度
          imgHeight: res.windowWidth * 0.4187, //user图片高度
          imgPageY: res.windowWidth * 0.1173, //user图片Y轴位置
          codeWidth: res.windowWidth * 0.192, //小程序码图片宽度
          codeHeight: res.windowWidth * 0.192, //小程序码图片高度
          codePageY: res.windowWidth * 1.12, //小程序码Y轴位置
          avatarPageY: res.windowWidth * 0.7947 + 168, //地址IconY轴位置
          titlePageY: res.windowWidth * 0.7947 + 180, //地址Y轴位置
          specPageY: res.windowWidth * 0.7947 + 115, //手机Y轴位置
          pricePageY: res.windowWidth * 0.7947 + 35, //姓名职位Y轴位置
          timePageY: res.windowWidth * 0.7947 + 150 //门店地址Y轴位置
        });
      }
    });
  ...
  createNewImg: function (lineNum) {
    ...
    //user图片
    ctx.arc(that.data.imgWidth / 2 + that.data.windowWidth * 0.3174, that.data.imgHeight / 2 + that.data.imgPageY, that.data.imgWidth / 2, 0, Math.PI * 2, false);
    ctx.clip();
    ctx.drawImage(
      that.data.userImg,
      that.data.windowWidth * 0.3174,
      that.data.imgPageY,
      that.data.imgWidth,
      that.data.imgHeight
    );
