// pages/templateDetail/tempCard/tempCardF/tempCardF.js
...
Page({
  data: {
    ...
    userImg: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602835410820redt_e300.png",  //user图片
    imgs: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/160125547050801.png", // 背景图
    name: "段杨杨",
    position: "销售顾问",
    phone: "136 9482 4093",
    phoneIcon: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602829422330redt_e300.png",
    storeAddress: "广汽丰田深圳兆方店",
    storeAddressIcon: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602829409511redt_e300.png",
    address: "广东省深圳市南山区丽水路1052号",
    addressIcon: "https://sixeco-redt.oss-cn-shenzhen.aliyuncs.com/redt-user-files/1602836397439redt_e300.png",
    iconWidth: 14,
    iconHeight: 14
  },
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({
          ...
          imgHeight: res.windowWidth * 0.4587, //user图片高度
          imgPageY: res.windowWidth * 0.976, //user图片Y轴位置
          codeWidth: res.windowWidth * 0.192, //小程序码图片宽度
          codeHeight: res.windowWidth * 0.192, //小程序码图片高度
          codePageY: res.windowWidth * 1.12, //小程序码Y轴位置
          avatarPageY: res.windowWidth * 0.976 + 108, //地址IconY轴位置
          titlePageY: res.windowWidth * 0.976 + 120, //地址Y轴位置
          specPageY: res.windowWidth * 0.976 + 70, //手机Y轴位置
          pricePageY: res.windowWidth * 0.976 + 35, //姓名职位Y轴位置
          timePageY: res.windowWidth * 0.976 + 95 //门店地址Y轴位置
        });
      }
    });
  ...
  drawSquare: function (ctx, height) {
    let that = this;
    let x=that.data.windowWidth * 0.0427,y=0,w=that.data.boxWidth,h=height,r=20
				 ctx.beginPath()
				 ...
				 // 剪切
				 ctx.clip()
				ctx.fillRect(0,0, that.data.boxWidth, height)
    ctx.drawImage(
      that.data.imgs,
  ...
  createNewImg: function (lineNum) {
    ...
    ctx.drawImage(
      that.data.userImg,
      that.data.windowWidth * 0.0423,
      that.data.imgPageY,
      that.data.windowWidth * 0.92,
      that.data.imgHeight
    );
    // 姓名
    ctx.setFontSize(18);
