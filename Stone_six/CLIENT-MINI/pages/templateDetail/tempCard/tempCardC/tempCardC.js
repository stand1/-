// pages/templateDetail/tempCard/tempCardC/tempCardC.js
...
Page({
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({
          ...
          normalPageX: res.windowWidth * 0.3174,
          ...
          imgWidth: res.windowWidth * 0.368, //user图片宽度
          imgHeight: res.windowWidth * 0.368, //user图片高度
          imgPageY: res.windowWidth * 0.538, //user图片Y轴位置
          codeWidth: res.windowWidth * 0.1227, //小程序码图片宽度
          codeHeight: res.windowWidth * 0.1227, //小程序码图片高度
          codePageY: res.windowWidth * 0.2667, //小程序码Y轴位置
          avatarPageY: res.windowWidth * 0.538 + res.windowWidth * 0.45 + 103, //地址IconY轴位置
          titlePageY: res.windowWidth * 0.538 + res.windowWidth * 0.45 + 115, //地址Y轴位置
          specPageY: res.windowWidth * 0.538 + res.windowWidth * 0.45 + 65, //手机Y轴位置
          pricePageY: res.windowWidth * 0.538 + res.windowWidth * 0.45, //姓名职位Y轴位置
          timePageY: res.windowWidth * 0.538 + res.windowWidth * 0.45 + 90 //门店地址Y轴位置
        });
      }
    });
  ...
  createNewImg: function (lineNum) {
    ...
    ctx.fillText(that.data.name, that.data.windowWidth * 0.5 - 0.5, that.data.pricePageY);
    ctx.fillText(that.data.name, that.data.windowWidth * 0.5, that.data.pricePageY-0.5);
    // 职位
    ctx.setFontSize(12);
    ctx.setFillStyle("#fff");
    ctx.fillText(
    ...
    //user图片
    ctx.arc(that.data.imgWidth / 2 + that.data.normalPageX, that.data.imgHeight / 2 + that.data.imgPageY, that.data.imgWidth / 2, 0, Math.PI * 2, false);
    ctx.clip();
    ctx.drawImage(
      that.data.userImg,
      that.data.normalPageX,
